...
__version__ = "7.0"         # Read way more about versioning here:
...
def corrected_revcomp_nlessseq2(seq1: str, seq2: str) -> str: 
    """
    takes two sequences with 'N's and checks if they are reverse complimented 
    returns the corrected seq1 (no N's) if they are 
    returns an empty string if they are not 
    assumes the second seq has no N's
    """ 
    out, seq1, seq2 = "", seq1.upper(), seq2.upper()[::-1]
    # if len(seq1) != len(seq2) or not validate_base_seq(seq1) or not validate_base_seq(seq2): return ""
    for x in range(len(seq1)): 
        if seq1[x] == 'N': 
            out += DNACOMPDICT[seq2[x]]
            continue 
        if seq1[x] != DNACOMPDICT[seq2[x]]: return ""
        out+=seq1[x]
    return out

def corrected_comp_nlessseq2(seq1: str, seq2: str) -> str:
    """
    this function will output a string 
    of the corrected first seq without any Ns 
    when compared to the second seq 
    it assumes they are of equal length, and the second seq has no N's
    """
    out, seq1, seq2 = "", seq1.upper(), seq2.upper()
    for x in range(len(seq1)):
        if seq1[x] == 'N': 
            out += seq2[x]
            continue 
        if seq1[x] != seq2[x]: return ""
        else: out += seq1[x]
    return out
...
if __name__ == "__main__":
    ...
    assert corrected_revcomp("ATGCGT", "ATGCGT") == "", "wrong corrected reverse compliment duplicate barcode"
    assert corrected_revcomp("NATCG", "CGNTA") == "TATCG", "wrong corrected reverse compliment for 'NATCG', 'CGNTA'"
    print("passed corrected reverse compliment tests!")
    assert corrected_comp_nlessseq2("ATCNG", "ATCAG") == "ATCAG", "wrong corrected comp for 'ATCNG', 'ATCAG'"
    print("passed corrected corrected_comp_nlessseq2 tests!")
    assert corrected_revcomp_nlessseq2("ATCNG", "CAGAT") == "ATCTG", "wrong corrected comp for 'ATCNG', 'CAGAT'"
